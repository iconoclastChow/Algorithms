...
"""
 四、快速排序，和前面三种思想上有较大不同，不再是外层循环次数下，内层循环实现将某一元素放至某位置。其核心思想是以某个值作为基准值，
...
"""
五、希尔排序，是插入排序的改进版，先以一定步长将列表分为多个子列表，然后对每个子列表插入排序。执行完成后，缩小步长继续，直至步长为0。
...
"""
六、归并排序，如名字一样，是将序列一边合并一边排序。其核心思想为：
